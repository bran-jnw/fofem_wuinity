...
int  NES_GetGrpCov (int ix, char cr_GrpNum[], char cr_CovTyp[], int *aiS_Def)
{
  *aiS_Def = 0;
  ...
  if ( !strcasecmp (sr_NES[ix].cr_Type,e_TyFSTS) )
    return 1; 
  else if ( !strcasecmp (sr_NES[ix].cr_Type,e_TyDuff) )
    return 2; 
  else if ( !strcasecmp (sr_NES[ix].cr_Type,e_TyCoar) )
    return 3; 
...
float f_Factor;
  ...
  for ( i = 0; i < 8; i++ ) {
    ...
    if ( i == 0  &&  strcasecmp (cr_Code,"CO2"))      { i_Ret = -2; goto Close; }
    if ( i == 1  &&  strcasecmp (cr_Code,"CO"))       { i_Ret = -3; goto Close; }
    if ( i == 2  &&  strcasecmp (cr_Code,"CH4"))      { i_Ret = -4; goto Close; }
    if ( i == 3  &&  strcasecmp (cr_Code,"NOx as NO")){ i_Ret = -5; goto Close; }
    if ( i == 4  &&  strcasecmp (cr_Code,"SO2"))      { i_Ret = -7; goto Close; }
    if ( i == 5  &&  strcasecmp (cr_Code,"PM2.5"))    { i_Ret = -8; goto Close; }
    if ( i == 6  &&  strcasecmp (cr_Code,"PM10"))     { i_Ret = -9; goto Close; }
  }
...
char *sav1, *sav2;
 ...
 for ( i = 0; i < 100000; i++ ) {
    ...
    r1 = strtok_r (Row1,",", &sav1);    /* Group Number */
	_Trim_LT (r1);
	strcpy (sr_NES[iX].cr_GrpNum,r1);

    r1 = strtok_r (sav1,",", &sav1);    /* Cover Type */
    _Trim_LT (r1);
    ...
    r1 = strtok_r (sav1,",", &sav1);    /* Emission Factor */
...
/*..............................................*/
  ...
  r1 = strtok_r (Row1,",", &sav1);
  _Trim_LT(r1);
  if ( strcasecmp (r1,"##") ){ /* See Note-2 above */
     i_Ret = -1;
	 goto Ret; }
...
/* Change - 10-9-14, DL want to add this into the file */
  r1 = strtok_r (sav1,",", &sav1);
  _Trim_LT(r1);
  if ( strcasecmp (r1,"units:g/kg") ){ 
     i_Ret = -1;
	 goto Ret; }

  r1 = strtok_r (sav1,",", &sav1);   /* 1st Name, well use it in a bit */

  r2 = strtok_r (Row2,",", &sav2);
  _Trim_LT(r2);
  if ( strcasecmp (r2, "Group #") ) {
	 i_Ret =  -2;
	 goto Ret; }

  r2 = strtok_r (sav2,",", &sav2);
  _Trim_LT(r2);
  if ( strcasecmp (r2, "# Cover Type") ) {   /* "# Description" */
	 i_Ret = -3;
	 goto Ret; }

  r2 = strtok_r (sav2,",", &sav2);
  _Trim_LT(r2);
  if ( strcasecmp (r2, "Type") ) {
	 i_Ret = -4;
	 goto Ret; }
  ...
  r2 = strtok_r (sav2,",", &sav2);  /* 1st Code we'll be using */
...
int i; 
	for ( i = 0; i < eC_NCT; i++ ) {
	  if ( !strcasecmp (sr_NCT[i].cr_Name,""))
		 break;
